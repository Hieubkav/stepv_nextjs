...
// Quản lý blocks của trang 'about' - tương tự home-blocks
...
export default function AboutBlocksPage() {
  ...
  const seed = useMutation(api.seed.seedAbout);
  ...
  const removeBlock = useMutation(api.pageBlocks.remove);
  ...
  async function ensureSeed() {
    try {
      ...
      if ((res as any)?.ok) toast.success("Khởi tạo dữ liệu trang About thành công");
    } catch {
  ...
  async function onDelete(b: Block) {
    if (!confirm(`Xóa block "${b.kind}"?`)) return;
    await removeBlock({ id: b._id as any });
    toast.success("Đã xóa block");
  }

  async function onBulkDelete() {
    if (selected.length === 0) return;
    if (!confirm(`Xóa ${selected.length} block đã chọn?`)) return;
    for (const id of selected) {
      await removeBlock({ id: id as any });
    }
    setSelected([]);
    toast.success(`Đã xóa ${selected.length} block`);
  }

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Khối giao diện - Trang About</h1>
        <p className="text-sm text-muted-foreground mt-1">Quản lý thứ tự, hiển thị và nội dung các block của trang Về chúng tôi.</p>
      </div>

      {!page && (
        <Card>
          <CardHeader>
            <CardTitle>Chưa có trang 'about'</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between gap-2">
              ...
              <Button asChild>
                <Link href="/dashboard/about-blocks/new">Thêm block</Link>
              </Button>
            </div>
          </CardHeader>
          <CardContent>
                ...
                <div className="divide-y rounded-md border">
                  ...
                  <div className="flex items-center gap-3 p-3 bg-muted/30">
                    ...
                    <div className="w-8 text-muted-foreground text-sm">#</div>
                      ...
                      <div className="w-44 text-sm">Hành động</div>
                  </div>
                    ...
                    <div key={String(b._id)} className="flex items-center gap-3 p-3">
                      ...
                      <div className="w-44 flex items-center gap-1">
                        <Button variant="outline" size="sm" asChild>
                           <Link href={`/dashboard/about-blocks/${String(b._id)}`}>Sửa</Link>
                         </Button>
                        ...
                        <Button variant="outline" size="icon" onClick={() => onDelete(b)} title="Xóa" className="text-destructive hover:bg-destructive hover:text-destructive-foreground">
